...
import type { SessionActor, User } from "../domain/auth/users";

export const authSessionCookieName = "ds_bags_session";
...
function sessionActor(session: { actorId?: string; actorName?: string }, user: User): SessionActor {
  const actorName = session.actorName?.trim() || user.displayName?.trim() || `同事${user.phone.slice(-4)}`;
  return {
    actorId: session.actorId || `legacy-${user.id}`,
    actorName
  };
}
