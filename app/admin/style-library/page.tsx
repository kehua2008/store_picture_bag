...
export default function StyleLibraryAdminPage() {
  ...
  async function refresh() {
    try {
      ...
      if (!response.ok) {
        handleAdminFailure("风格库读取失败", response, body);
        return;
      }
      ...
      setStatus(describeNetworkFailure("风格库读取失败", error));
    }
  }

  function handleAdminFailure(prefix: string, response: Response, body: Record<string, unknown>) {
  ...
  async function uploadSamples(event: FormEvent<HTMLFormElement>) {
    ...
    const body = await readJsonRecord(response);
    if (!response.ok) {
      handleAdminFailure("上传失败", response, body);
      return;
    }
    ...
    if (sampleIds.length) {
      ...
      const candidateBody = await readJsonRecord(candidateResponse);
      if (!candidateResponse.ok) {
        handleAdminFailure("图片已上传，但候选批次创建失败", candidateResponse, candidateBody);
        await refresh();
        return;
      }
      setCandidateName("");
    ...
    clearQueuedFiles();
    const uploadedSamples = Array.isArray(body.samples) ? body.samples as StyleSample[] : [];
    setStatus(`已上传 ${uploadedSamples.length} 张候选图，并创建 Codex 分析批次。下一步导出 zip 给 Codex 聚类。`);
    await refresh();
  }
  ...
  async function rebuildBoard(group: { platform: string; category: string; imageType: string; styleName: string }) {
    ...
    const body = await readJsonRecord(response);
    if (!response.ok) {
      handleAdminFailure("风格板生成失败", response, body);
      return;
    }
    const board = body.board as StyleBoard;
    setStatus(`${board.styleName} 已发布到前台经典风格区`);
    await refresh();
  }
  ...
  async function createCandidateBatch() {
    ...
    const body = await readJsonRecord(response);
    if (!response.ok) {
      handleAdminFailure("候选批次创建失败", response, body);
      return;
    }
    setCandidateName("");
    const batch = body.batch as StyleCandidateBatch;
    setStatus(`已创建候选批次：${batch.name}`);
    await refresh();
  }
  ...
  async function importAnalysisResult() {
    ...
    const body = await readJsonRecord(response);
    if (!response.ok) {
      handleAdminFailure("分析结果导入失败", response, body);
      return;
    }
    setAnalysisJson("");
    const importedBoards = Array.isArray(body.boards) ? body.boards as StyleBoard[] : [];
    setStatus(`已导入 ${importedBoards.length} 个待发布风格板`);
    await refresh();
  }
  ...
  async function updateBoardPublishState(board: StyleBoard, patch: { status?: StyleBoardStatus; showOnHome?: boolean; displayOrder?: number }) {
    ...
    const body = await readJsonRecord(response);
    if (!response.ok) {
      handleAdminFailure("风格板发布状态更新失败", response, body);
      return;
    }
    const updatedBoard = body.board as StyleBoard;
    setStatus(`${updatedBoard.styleName} 已更新`);
    await refresh();
  }
  ...
  async function updateSampleStatus(id: string, nextStatus: StyleSampleStatus) {
    ...
    const body = await readJsonRecord(response);
    if (!response.ok) {
      handleAdminFailure("样本状态更新失败", response, body);
      return;
    }
  ...
  async function updateSample(id: string, patch: Partial<Pick<StyleSample, "platform" | "category" | "imageType" | "styleName">>) {
    ...
    const body = await readJsonRecord(response);
    if (!response.ok) {
      handleAdminFailure("样本修改失败", response, body);
      return;
    }
    setSamples((current) => current.map((item) => (item.id === id ? body.sample as StyleSample : item)));
    setStatus("样本归类已更新");
  }
    ...
    <main className="adminShell">
      ...
      <section className="adminWorkflowGrid">
        <form className="adminPanel uploadPanel" onSubmit={uploadSamples}>
          ...
          <div className="adminFormGrid">
            <label>
              候选批次名
              <input value={candidateName} onChange={(event) => setCandidateName(event.target.value)} placeholder="例如：6月女包优秀主图 / 高点击风格混合样本" />
            </label>
          </div>
          <label>
            来源备注
            <textarea rows={3} value={sourceNote} onChange={(event) => setSourceNote(event.target.value)} placeholder="例如：天猫通勤包优秀店铺截图 / 品牌官网Lookbook / 自己筛选的好图" />
          </label>
        ...
        <section className="adminPanel">
          ...
          <div className="analysisSampleList">
            ...
            {!pendingSamples.length ? <em>当前没有需要人工逐张归类的图片。请在下方候选批次中导出 zip 给 Codex 分析。</em> : null}
          </div>
        </section>
      </section>
      ...
      <section className="adminPanel">
        ...
        <div className="adminFormGrid">
          <label>
            批次名称
            <input value={candidateName} onChange={(event) => setCandidateName(event.target.value)} placeholder="例如：2026-06 男包主图优秀样本" />
          </label>
        ...
        <div className="styleGroupList styleGroupCards">
          ...
          {!candidateBatches.length ? <em>还没有候选批次。确认样本后可在这里创建分析批次。</em> : null}
        </div>
        <label>
      ...
      <section className="adminPanel">
        ...
        <div className="styleGroupList styleGroupCards">
          ...
          {!grouped.length ? <em>确认样本后，这里会出现可发布的风格分组。</em> : null}
        </div>
      </section>
      ...
      <section className="adminPanel">
        ...
        <div className="styleBoardGrid">
          ...
          {!boards.length ? <em>还没有风格板。上传样本后点击重建并发布。</em> : null}
        </div>
      </section>
