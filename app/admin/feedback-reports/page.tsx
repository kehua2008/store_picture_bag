...
export default function FeedbackReportsAdminPage() {
  ...
  async function refresh() {
    ...
    try {
      ...
      if (!response.ok) {
        const message = describeRequestFailure("反馈读取失败", response, body);
        setLoadError(message);
        setStatus(message);
        if (response.status === 401 || response.status === 403) redirectToAdminLogin();
        return;
      }
      setReports(Array.isArray(body.reports) ? body.reports as FeedbackReport[] : []);
      setStatus("反馈收件箱已同步");
    } catch (error) {
      const message = describeNetworkFailure("反馈读取失败", error);
      setLoadError(message);
      setStatus(message);
    }
  }
  ...
  async function reviewReport(id: string, nextStatus: FeedbackStatus) {
    ...
    const body = await readJsonRecord(response);
    if (!response.ok) {
      setStatus(describeRequestFailure("更新失败", response, body));
      if (response.status === 401 || response.status === 403) redirectToAdminLogin();
      return;
    }
    setReports((current) => current.map((report) => report.id === id ? body.report as FeedbackReport : report));
  ...
  return (
    <main className="adminShell">
      ...
      <section className="adminStatsGrid feedbackStatsGrid">
        <article><span>待审核</span><strong>{pendingReports.length}</strong><em>新反馈</em></article>
        <article><span>有效问题</span><strong>{validReports.length}</strong><em>待跟进</em></article>
        <article><span>已处理</span><strong>{resolvedReports.length}</strong><em>完成闭环</em></article>
        <article><span>无效反馈</span><strong>{invalidReports.length}</strong><em>已过滤</em></article>
      </section>

      <section className="adminPanel">
        <div className="adminPanelHeader"><span>01</span><strong>待审核反馈</strong></div>
        {loadError ? <em className="adminStatusLine">{loadError}</em> : null}
        <FeedbackReportList reports={pendingReports} notes={notes} setNotes={setNotes} reviewReport={reviewReport} emptyLabel={loadError ? "" : "暂无反馈。"} />
        <p className="adminStatusLine">{status}</p>
      </section>

      <section className="adminPanel">
        <div className="adminPanelHeader"><span>02</span><strong>已归档反馈</strong></div>
        <FeedbackReportList reports={[...validReports, ...resolvedReports, ...invalidReports]} notes={notes} setNotes={setNotes} reviewReport={reviewReport} emptyLabel={loadError ? "" : "暂无反馈。"} />
      </section>
    </main>
  );
}
...
function FeedbackReportList({
  ...
  if (!reports.length) return emptyLabel ? <em className="feedbackEmptyState">{emptyLabel}</em> : null;
