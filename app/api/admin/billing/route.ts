...
export async function GET(request: Request) {
  ...
  const summary = {
    ...
    totalConsumedCredits: Math.abs(summaryEntries.filter(isConsumptionEntry).reduce((sum, entry) => sum + entry.deltaBalanceCredits + entry.deltaFrozenCredits, 0)),
    ...
    todayConsumedCredits: Math.abs(summaryEntries.filter((entry) => isConsumptionEntry(entry) && new Date(entry.createdAt) >= todayStart).reduce((sum, entry) => sum + entry.deltaBalanceCredits + entry.deltaFrozenCredits, 0))
  };

  return NextResponse.json({ summary, ledgerEntries, accounts: data.accounts, orders: data.orders });
}

function isConsumptionEntry(entry: { type: CreditLedgerEntryType }): boolean {
  return entry.type === "generation_debit" || entry.type === "style_analysis_debit" || entry.type === "usage_debit";
}
