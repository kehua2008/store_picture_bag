...
export default function RechargePage() {
  ...
  useEffect(() => {
    void refreshCurrentUserAndOrders();
  }, []);
  ...
  async function refreshCurrentUserAndOrders() {
    const response = await fetch("/api/auth/me");
    const body = await response.json().catch(() => ({} as CurrentUserResponse));
    if (!response.ok || !body.user?.id) {
      setCustomerId("");
      setAccount(undefined);
      setOrders([]);
      setOrderStatus("请先返回工作台登录账号，再提交充值申请");
      return;
    }
    setCustomerId(body.user.id);
    if (body.account) setAccount(body.account);
    await refreshRechargeOrders(body.user.id);
  }

  async function refreshRechargeOrders(nextCustomerId = customerId) {
    if (!nextCustomerId) return;
    const response = await fetch(`/api/recharge-orders?customerId=${nextCustomerId}`);
    const body = await response.json().catch(() => ({}));
    if (!response.ok) {
      setOrderStatus(`读取充值记录失败：${body.error ?? "unknown_error"}`);
      return;
    }
      ...
      <section className="rechargeShell">
        <div className="creditHero">
          <div>
            ...
            <h1>按生成量充值，用多少扣多少</h1>
            <p>1 算力点按 0.10 元折算，充值档位越高，单张图片和单条视频扣点越低。</p>
          </div>
