...
const pm2AppName = process.env.PM2_APP_NAME || "store-picture-bag";
...
function printDataSizes(baseDir) {
  ...
  for (const target of [
    baseDir,
    path.join(baseDir, "generated-images"),
    path.join(baseDir, "generated-videos"),
    path.join(baseDir, "generation-jobs.json"),
    path.join(baseDir, "video-jobs.json")
  ]) {
